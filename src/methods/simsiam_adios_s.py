...
class SimSiam_ADIOS_S(BaseADIOSModel):
    def __init__(
        self,
        output_dim: int,
        proj_hidden_dim: int,
        ...
        **kwargs,
        ...
        """Implements SimSiam (https://arxiv.org/abs/2011.10566) with ADIOSs(ADIOS single).
    ...
    def add_model_specific_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        parent_parser = super(SimSiam_ADIOS_S, SimSiam_ADIOS_S).add_model_specific_args(parent_parser)
    ...
    def mask_forward(self, batch: Sequence[Any]) -> torch.Tensor:
        ...
        # generate masks
        enc_feat_orig = self.mask_encoder(x_orig)
        masks_orig = self.mask_head(enc_feat_orig)
        enc_feat_transformed = self.mask_encoder(x_transformed)
        masks_transformed = self.mask_head(enc_feat_transformed)
        # get random mask for each sample in the batch
        chosen_mask = torch.randint(low=0, high=self.N, size=(self.batch_size, ))
        mask_orig = torch.stack([masks_orig[i, c, ...] for i, c in enumerate(chosen_mask)], dim=0).unsqueeze(1)
        mask_transformed = torch.stack([masks_transformed[i, c, ...] for i, c in enumerate(chosen_mask)], dim=0).unsqueeze(1)
        ...
        summed_mask = []
        for mask in [masks_orig, masks_transformed]:
            mask_penalty = mask.sum([-1, -2]) / self.img_size ** 2
            summed_mask.extend(torch.chunk(mask_penalty.view(-1), (self.N)*self.batch_size, dim=0))
    ...
    def inpaint_forward(self, batch: Sequence[Any]) -> torch.Tensor:
        ...
        # generate masks
        enc_feat_orig = self.mask_encoder(x_orig)
        masks_orig = self.mask_head(enc_feat_orig).detach()
        enc_feat_transformed = self.mask_encoder(x_transformed)
        masks_transformed = self.mask_head(enc_feat_transformed).detach()
        # get random mask for each sample in the batch
        chosen_mask = torch.randint(low=0, high=self.N, size=(self.batch_size, ))
        mask_orig = torch.stack([masks_orig[i, c, ...] for i, c in enumerate(chosen_mask)], dim=0).unsqueeze(1)
        mask_transformed = torch.stack([masks_transformed[i, c, ...] for i, c in enumerate(chosen_mask)], dim=0).unsqueeze(1)
