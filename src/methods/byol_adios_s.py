...
class BYOL_ADIOS_S(BaseMomentumADIOSModel):
    def __init__(
        self,
        output_dim: int,
        proj_hidden_dim: int,
        ...
        **kwargs,
        ...
        """Implements BYOL (https://arxiv.org/abs/2006.07733) with ADIOSs. In each
        forward pass, the masking function generates N masks from an input image.
    ...
    def add_model_specific_args(parent_parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        parent_parser = super(BYOL_ADIOS_S, BYOL_ADIOS_S).add_model_specific_args(parent_parser)
    ...
    def inpaint_forward(self, batch: Sequence[Any]) -> torch.Tensor:
        ...
        mask = torch.stack([masks[i, c, ...] for i, c in enumerate(chosen_mask)],
                           dim=0).unsqueeze(1).detach()
        ...
        return byol_loss_func(p1, z2_momentum) + byol_loss_func(p2, z1_momentum)
