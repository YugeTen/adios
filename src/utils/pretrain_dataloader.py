...
from src.args.utils import N_CLASSES_PER_DATASET
...
class NCropAugmentation:
    ...
    def __call__(self, x: Image, **kwargs) -> List[torch.Tensor]:
        ...
        if self.one_transform_per_crop:
            return [transform(x, **kwargs) for transform in self.transform]
        else:
            return [self.transform(x, **kwargs) for _ in range(self.n_crops)]
...
class ClevrTransform:
    def __init__(
        ...
        """Class that applies clevr transformations.
        ...
        self.transform_normalise = transforms.Normalize(mean=mean, std=std)

    def __call__(self, x: Image, no_color: bool=False) -> torch.Tensor:
        if no_color:
            return self.transform_to_tensor(self.transform_base(x))
        else:
            return self.transform_normalise(self.transform_to_tensor(self.transform_color(self.transform_base(x))))
...
def prepare_datasets(
    ...
    if train_dir is None:
        if dataset == "imagenet100s":
            train_dir = "imagenet100/train"
        else:
            train_dir = f"{dataset}/train"

    if dataset in ["cifar10", "cifar100"]:
        DatasetClass = getattr(torchvision.datasets, dataset.upper())
